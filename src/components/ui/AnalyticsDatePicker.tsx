    ...
    <div className="flex items-center justify-between gap-3 relative">
      ...
      <div className="relative" ref={dropdownRef}>
        <button
          ...
          className="border border-[#0B0A07] px-2 py-3 rounded-2xl flex items-center gap-2 hover:bg-gray-50 transition-colors min-w-[100px] justify-center"
        >
      ...
      {/* MONTH SELECTOR */}
      <div className="border border-[#0B0A07] px-3 py-2 rounded-2xl flex items-center gap-2">
        ...
        {selectedMonth > 0 && (
          ...
          >
            <ArrowLeftIcon className="w-3.5 h-3.5 text-[#0B0A07]" />
          </button>
        )}

        {/* Scrollable months container - shows 4 months at a time */}
        <div
          ...
          style={{
            ...
            width: "calc(4 * 56px + 3 * 4px)", // 4 months × 56px width + 3 gaps × 4px
          }}
        >
          {months.map((month, monthIndex) => {
            ...
            return (
              <button
                ...
                className={`px-3 py-1.5 rounded-xl text-sm font-medium tracking-tight transition-all duration-300 flex-shrink-0 w-[56px] ${
                  isSelected
        ...
        {selectedMonth < 11 && (
          <button
            ...
            className="p-1 hover:bg-gray-100 rounded transition-colors flex-shrink-0"
          >
            <ArrowRightIcon className="w-3.5 h-3.5 text-[#0B0A07]" />
          </button>
        )}
      </div>
    </div>
